              ...
              <TableRow key={plan.planKey}>
                ...
                <TableCell className="text-right">
                  <Button
                    ...
                    onClick={() => mutation.mutate({ planKey: plan.planKey, body: d })}
                  >
                    Save
                  </Button>
                </TableCell>
...
function WorkspacesTab() {
  ...
  const usageQuery = useQuery({
    queryKey: ["ai-token-workspaces", workspaceFilter, sortBy],
    queryFn: () =>
      listAiTokenWorkspaceUsage({
        limit: 50,
        sortBy,
  ...
  const ledgerQuery = useQuery({
    queryKey: ["ai-token-ledger", ledgerTarget],
    queryFn: () => getAiTokenLedger(ledgerTarget as string),
  ...
  return (
    <>
          ...
          <TableBody>
            {(usageQuery.data?.workspaces ?? []).map((row) => (
              <TableRow key={row.id}>
                ...
                <TableCell>{row.allocatedTokens === -1 ? "Unlimited" : row.allocatedTokens.toLocaleString()}</TableCell>
                ...
                <TableCell className="flex justify-end gap-2">
                  <Button size="sm" variant="outline" onClick={() => setLedgerTarget(row.workspaceId)}>
                    View ledger
                  </Button>
      ...
      <Dialog open={Boolean(ledgerTarget)} onOpenChange={(open) => !open && setLedgerTarget(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="font-mono text-sm">Ledger — {ledgerTarget}</DialogTitle>
          </DialogHeader>
...
function FeaturesTab() {
  const queryClient = useQueryClient();
  const featuresQuery = useQuery({ queryKey: ["ai-feature-registry"], queryFn: listAiFeatureRegistry });

  const mutation = useMutation({
    mutationFn: ({ featureKey, isMetered }: { featureKey: string; isMetered: boolean }) =>
      updateAiFeatureMetering(featureKey, { isMetered }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["ai-feature-registry"] });
    },
    onError: (err: Error) => toast.error(err.message),
  });
...
function LedgerTab() {
  ...
  const ledgerQuery = useQuery({
    queryKey: ["ai-token-ledger-all", workspaceFilter, entryType, from, to],
    queryFn: () =>
      listAiTokenLedger({
        workspaceId: workspaceFilter.trim() || undefined,
    ...
    <>
      <div className="mb-4 flex flex-wrap items-end gap-3">
        ...
        <div className="space-y-1">
          ...
          <Select value={entryType} onValueChange={(v) => setEntryType(v as AiTokenLedgerEntryType | "ALL")}>
