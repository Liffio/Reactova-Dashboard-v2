...
import { useAffiliateCalculatorConfig } from "@/hooks/useAffiliateCalculator";
...
export function AffiliateProfitCalculator() {
  ...
  if (isLoading) {
    return (
      <section className="surface-card overflow-hidden p-6 sm:p-8 space-y-4">
  ...
  if (isError || !config) {
    return (
      <section className="surface-card overflow-hidden p-6 text-sm text-muted-foreground">
        Unable to load earnings estimate. Refresh the page to try again.
      </section>
    );
  }

  const { currency, currencyLabel, planMonthly, monthlyCommissionPerReferral, commissionRatePercent } =
    config;

  return (
    <section className="affiliate-earnings-calculator relative overflow-hidden rounded-2xl border border-primary/20 surface-card shadow-lg">
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-primary/12 via-transparent to-violet/10" />
      ...
      <div className="relative px-5 py-6 sm:px-8 sm:py-8">
        ...
        <p className="mt-8 text-4xl sm:text-5xl font-extrabold text-foreground tabular-nums tracking-tight">
