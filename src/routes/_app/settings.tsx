  ...
  return (
    <div className="rounded-2xl border bg-card p-6 shadow-soft space-y-4">
      ...
      {notifQuery.isLoading ? (
        ...
        <p className="text-sm text-muted-foreground">
          No notification preferences found for this workspace.
        </p>
      ) : (
            ...
            <div key={item.type} className="flex items-center justify-between py-3">
              ...
              <Switch
                ...
                onCheckedChange={(checked) =>
                  updateMutation.mutate({ type: item.type, isEnabled: checked })
                }
...
function TeamSettings() {
   ...
   *
   * Limit 100 rather than the usual 25: this panel checks invite duplicates against the loaded
   * emails, and plan caps top out well below 100 members (see `PLAN_TEAM_LIMITS`), so one page
   * covers every real workspace. The *count* below still comes from the server total, so the plan
   * check stays correct even if a workspace somehow exceeds it.
   */
  ...
  // plan limit whenever the page happened to be smaller than the cap.
  const memberCount = memberList.total;
  ...
  const inviteEmailError = email
    ? emailError(email) || duplicateAliasError(email, existingEmails)
    : null;

  const invalidate = () => {
    void Promise.all([membersQuery.refetch(), invitesQuery.refetch()]);
  };
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border bg-card p-6 shadow-soft space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <Button
            size="sm"
            disabled={Boolean(inviteEmailError) || createInviteMutation.isPending || atLimit}
                ...
                <div key={m.user.email} className="flex items-center gap-3 px-6 py-3">
                  ...
                  {isOwner ? (
                    ...
                    <select
                      value={m.role.key}
                      onChange={(e) =>
                        updateRoleMutation.mutate({ userId: m.user.id, roleKey: e.target.value })
                      }
