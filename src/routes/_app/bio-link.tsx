...
const bioTemplates: Array<{ id: string; name: string; style: TemplateStyle }> = [
  ...
  },
  {
    ...
    style: {
      ...
      backgroundColor: "#C4D39A",
      backgroundColorTo: "#C4D39A",
      textColor: "#111827",
      cardStyle: "outline",
      cardColor: "#A3B18A",
      cardOpacity: 100,
  ...
  },
  {
    ...
    style: {
      ...
      backgroundColorTo: "#4C1D95",
      textColor: "#EDE9FE",
      cardStyle: "glass",
      cardColor: "#312E81",
      cardOpacity: 58,
      fontFamily: "playfair",
      buttonTextColor: "#DDD6FE",
      buttonRadius: 18,
      buttonBorderWidth: 1,
      buttonShadow: true,
      socialLayout: "horizontal",
  ...
  },
  {
    ...
    style: {
      ...
      backgroundColorTo: "#166534",
      textColor: "#ECFDF5",
      cardStyle: "solid",
      cardColor: "#14532D",
      cardOpacity: 78,
      fontFamily: "space-grotesk",
      buttonTextColor: "#BBF7D0",
      buttonRadius: 14,
      buttonBorderWidth: 0,
      buttonShadow: false,
      socialLayout: "horizontal",
...
function getPatternStyle(pattern: BackgroundPattern, opacity = 0.13): React.CSSProperties {
  ...
  switch (pattern) {
    ...
    case "circles":
      return {
        backgroundImage: `radial-gradient(circle, transparent 32%, ${c} 33%, ${c} 36%, transparent 37%)`,
        backgroundSize: "36px 36px",
      };
    case "crosshatch":
      return {
        backgroundImage: `repeating-linear-gradient(0deg, ${c} 0, ${c} 1px, transparent 0, transparent 50%), repeating-linear-gradient(90deg, ${c} 0, ${c} 1px, transparent 0, transparent 50%)`,
        backgroundSize: "22px 22px",
      };
    case "hexdots":
      return {
        backgroundImage: `radial-gradient(${c} 1.5px, transparent 1.5px), radial-gradient(${c} 1.5px, transparent 1.5px)`,
...
function initDraft(profile: BioLinkProfile, workspaceId: string): FullDraft {
  return {
    ...
    backgroundPattern:
      (localStorage.getItem(`biolink-pattern-${workspaceId}`) as BackgroundPattern) ?? "none",
        ...
        <div className="flex-1 min-w-0 space-y-4 p-5 lg:p-6 pb-20">
          ...
          {isCreatorProgramMember && (
            <div className="rounded-xl border border-primary/30 bg-primary/10 px-4 py-3 text-xs text-primary">
              As a Creator Program member, the "Powered by @Liffio" badge must stay visible on this
              page — it can't be hidden while your membership is active.
            </div>
          )}
          ...
          <SectionCard
            ...
            action={
              <span className="text-[11px] text-muted-foreground">
                {socials.length} social{socials.length !== 1 ? "s" : ""}
              </span>
            }
          >
            <SocialsSection
      ...
      <AlertDialog open={resetOpen} onOpenChange={setResetOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              All appearance settings will be reset to defaults. Your links and socials will not be
              affected.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function PhonePreview({
  ...
  const patternStyle = useMemo(
    () => getPatternStyle(draft.backgroundPattern),
  ...
  const btnStyle = useMemo((): React.CSSProperties => {
    ...
    if (draft.buttonStyle === "outlined")
      return {
        ...
        borderRadius: radius,
        border: `${Math.max(draft.buttonBorderWidth, 1)}px solid ${draft.accentColor}`,
        boxShadow: shadow,
        fontFamily,
        fontSize: "11px",
        fontWeight: 500,
    ...
    return {
      backgroundColor: `${draft.accentColor}28`,
      color: draft.buttonTextColor,
      borderRadius: radius,
      border: "none",
      boxShadow: shadow,
      ...
      >
        ...
        <div className="absolute inset-0 rounded-[30px] overflow-hidden" style={bgStyle}>
          ...
          <div className="absolute inset-0 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
            <div
              className="flex flex-col items-center px-4 pt-10 pb-8 text-center"
              style={{ color: draft.textColor, fontFamily }}
            >
              {/* Avatar */}
              <div className="mt-2 mb-2.5">
                {draft.avatarUrl && isValidUrl(draft.avatarUrl) ? (
                  ...
                  <div
                    ...
                    style={{ backgroundColor: draft.accentColor, color: draft.buttonTextColor }}
                  >
                    {draft.displayName?.[0]?.toUpperCase() ?? "?"}
                  </div>
                )}
              </div>
              <p className="font-bold text-[13px] leading-tight mb-1" style={{ fontFamily }}>
                {draft.displayName || "Your Name"}
              </p>
              {draft.bio && (
                <p className="text-[10px] opacity-65 mb-3.5 leading-relaxed max-w-[190px]">
                  {draft.bio}
                </p>
              )}
              ...
              {(draft.sectionOrder ?? ["links", "socials"]).map((section) =>
                section === "links" ? (
                  <div key="links" className="w-full space-y-1.5 mb-3">
                    ...
                    {links.length === 0 && (
                      <div style={{ ...btnStyle, opacity: 0.35 }}>Your link here</div>
                    )}
                  </div>
                ) : (
                  <div
                    key="socials"
                    className={`flex mb-3 ${draft.socialLayout === "horizontal" ? "flex-row gap-2.5 flex-wrap justify-center" : "flex-col gap-1.5 w-full"}`}
                  >
                    {socials.slice(0, 8).map((s) =>
                      draft.socialLayout === "horizontal" ? (
                        <div
                          ...
                          style={{ backgroundColor: `${draft.accentColor}22` }}
                        >
                          <SocialIcon
                            icon={s.icon}
                            style={{ color: draft.accentColor, width: 16, height: 16 }}
                          />
                        </div>
                      ) : (
                        <div
                          ...
                          style={{
                            backgroundColor: `${draft.cardColor}55`,
                            color: draft.textColor,
                          }}
                        >
                          <SocialIcon
                            icon={s.icon}
                            style={{ color: draft.textColor, width: 12, height: 12, flexShrink: 0 }}
                          />
                          <span>{s.label}</span>
                        </div>
                      ),
                    ...
                    {socials.length === 0 && (
                      <div
                        className="h-8 w-8 rounded-full flex items-center justify-center opacity-30"
                        style={{ backgroundColor: `${draft.accentColor}22` }}
                      >
    ...
    <div className="grid gap-3.5 sm:grid-cols-2">
      <div className="space-y-1.5">
        ...
        <Input
          ...
          onChange={(e) => set("displayName", e.target.value.slice(0, LIMITS.displayName.max))}
        ...
        <Label className="text-xs flex items-center justify-between">
          <span className="flex items-center gap-1.5">
            ...
            <BioTextAssist
              ...
              onApply={(next) => set("bio", next)}
            />
          </span>
          <span className="text-muted-foreground font-normal">
            {draft.bio.length}/{LIMITS.bioLinkBio.max}
          </span>
        </Label>
        <textarea
      ...
      <div className="space-y-1.5 sm:col-span-2">
        ...
        <div className="flex items-center rounded-lg border bg-background overflow-hidden">
          <span className="px-3 text-xs text-muted-foreground border-r py-2.5 bg-muted/30 shrink-0 whitespace-nowrap">
            bio.liffio.com/
          </span>
          <Input
            value={draft.slug}
            onChange={(e) =>
              set(
                ...
                e.target.value
                  .toLowerCase()
                  .replace(/[^a-z0-9-]/g, "")
                  .slice(0, LIMITS.bioLinkSlug.max),
              )
            }
            placeholder="your-brand"
            maxLength={LIMITS.bioLinkSlug.max}
        ...
        {draft.slug && draft.slug.length < LIMITS.bioLinkSlug.min && (
          <p className="text-[11px] text-destructive">
            Slug must be at least {LIMITS.bioLinkSlug.min} characters.
          </p>
        )}
...
function LinksSection({
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, title, url }: { id: string; title: string; url: string }) =>
      updateBioLinkItem(workspaceId, id, { title, url }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-2.5">
      {links.length === 0 && (
        <div className="rounded-xl border border-dashed bg-muted/20 p-7 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Add links to appear on your bio page.
          </p>
        </div>
      )}
      ...
      <AnimatePresence initial={false}>
        {links.map((link) => (
          <motion.div
            ...
            animate={{
              opacity: draggingId === link.id ? 0.45 : 1,
              height: "auto",
              scale: draggingId === link.id ? 1.015 : 1,
            }}
            ...
            onDragOver={(e) => {
              ...
              if (draggingId && draggingId !== link.id)
                onLinks(reorder(links, draggingId, link.id));
            }}
            onDrop={() => {
              setDraggingId(null);
              reorderMutation.mutate(links.map((i) => i.id));
            }}
      ...
      <LinkDialog
        ...
        onSubmit={(d) => editing && updateMutation.mutate({ id: editing.id, ...d })}
      />
      ...
      <AlertDialog open={Boolean(deleting)} onOpenChange={(o) => !o && setDeleting(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This link will be removed from your bio page.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
...
function SocialsSection({
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, ...body }: { id: string } & BioLinkSocialInput) =>
      updateBioLinkSocial(workspaceId, id, body),
    onSuccess: () => {
    ...
    <div className="space-y-2.5">
      {socials.length === 0 && (
        <div className="rounded-xl border border-dashed bg-muted/20 p-7 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Add social links to display on your bio page.
          </p>
        </div>
      )}
      ...
      <AnimatePresence initial={false}>
        {socials.map((social) => (
          <motion.div
            ...
            initial={{ opacity: 0, height: 0, marginBottom: 0 }}
            animate={{
              opacity: draggingId === social.id ? 0.45 : 1,
              height: "auto",
              scale: draggingId === social.id ? 1.015 : 1,
            }}
            exit={{ opacity: 0, height: 0, marginBottom: 0 }}
            transition={{ type: "spring", stiffness: 380, damping: 32 }}
            draggable
            ...
            onDragOver={(e) => {
              e.preventDefault();
              if (draggingId && draggingId !== social.id)
                onSocials(reorder(socials, draggingId, social.id));
            }}
            onDrop={() => {
              setDraggingId(null);
              reorderMutation.mutate(socials.map((i) => i.id));
            }}
      ...
      <AlertDialog open={Boolean(deleting)} onOpenChange={(o) => !o && setDeleting(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This social will be removed from your bio page.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5">
          {bioTemplates.map((t) => (
            <button
              ...
              style={{
                background:
                  t.style.backgroundType === "gradient"
            ...
            >
              <span className="absolute inset-x-0 bottom-0 flex items-center justify-center pb-0.5">
                <span className="text-[8px] font-medium px-1 py-px bg-black/50 text-white backdrop-blur-sm rounded leading-tight truncate max-w-full">
                  {t.name}
                </span>
              </span>
            </button>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <Label className="text-xs">
              {draft.backgroundType === "gradient" ? "From color" : "Color"}
            </Label>
            <div className="flex items-center gap-2">
              <input
                type="color"
                ...
                className="h-8 w-8 rounded-md border cursor-pointer p-0.5 bg-background"
              />
              <Input
                value={draft.backgroundColor}
                onChange={(e) => set("backgroundColor", e.target.value)}
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => set("backgroundColorTo", e.target.value)}
                  className="h-8 w-8 rounded-md border cursor-pointer p-0.5 bg-background"
                />
                <Input
                  value={draft.backgroundColorTo}
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-7 gap-1.5">
            {BACKGROUND_PATTERNS.map((p) => (
              ...
              >
                {p.id !== "none" && (
                  <div className="absolute inset-0" style={getPatternStyle(p.id, 0.45)} />
                )}
                <span className="absolute bottom-0 left-0 right-0 text-[7px] text-center text-white/60 pb-0.5">
                  {p.name}
                </span>
              </button>
            ))}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              onChange={(e) => set("buttonBorderWidth", Number(e.target.value))}
              className="w-full accent-primary"
            />
          </div>
        </div>
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              value={linkTitle}
              onChange={(e) => setLinkTitle(e.target.value.slice(0, LIMITS.linkTitle.max))}
              maxLength={LIMITS.linkTitle.max}
            />
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              value={url}
              onChange={(e) => setUrl(e.target.value.slice(0, LIMITS.url.max))}
              maxLength={LIMITS.url.max}
            />
        ...
        <DialogFooter>
          ...
          <Button
            disabled={
              isPending ||
              Boolean(lengthError(linkTitle, "Title", LIMITS.linkTitle)) ||
              Boolean(urlError(url, { required: true, max: LIMITS.url.max }))
            }
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              value={label}
              onChange={(e) => setLabel(e.target.value.slice(0, LIMITS.socialLabel.max))}
          ...
          <div className="space-y-2">
            ...
            {url && urlError(url, { required: true, max: LIMITS.url.max }) && (
              <p className="text-xs text-destructive">
                {urlError(url, { required: true, max: LIMITS.url.max })}
              </p>
            )}
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              <Input
                placeholder="instagram"
                value={icon}
                onChange={(e) => setIcon(e.target.value.slice(0, 30))}
                maxLength={30}
              />
            </div>
          </div>
        ...
        <DialogFooter>
          ...
          <Button
            disabled={
              isPending ||
              Boolean(lengthError(label, "Label", LIMITS.socialLabel)) ||
              Boolean(urlError(url, { required: true, max: LIMITS.url.max }))
            }
            onClick={() =>
              onSubmit({
